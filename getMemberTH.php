...
function getTH($id, $filtered, $transactionType, $searchID){
    global $db;
    ...
    if($filtered == 'yes' && $transactionType != 'null'){
        ...
        $stmt = $db->prepare($sql);
        $stmt->bind_param("siiii",$transactionType ,$id, $id, $searchID, $searchID);
        ...
        $stmt = $db->prepare($sql);
        $stmt->bind_param("iiii",$id, $id, $searchID, $searchID);
